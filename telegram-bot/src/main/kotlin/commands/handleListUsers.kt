...
suspend fun handleListUsers(user: User, bot: TelegramBot, chat: Chat) {
    ...
    runCatching {
                ...
                Users.selectAll()
                    ...
                    .map { row ->
                        UserInfo(
                            ...
                            source = "Группа: ${
                                targetGroups.find {
                                    row[Users.description].contains(
                                        it,
                                        true
                                    )
                                } ?: "Неизвестно"
        ...
        // Store session and display first page
        val sortedList = userList.sortedBy { it.birthDate }
        displayUserListPage(chat.id, bot, sortedList, 0, null)

        // Store the session (messageId will be set when user clicks pagination buttons)
...
// Callback handler for list users pagination
...
suspend fun handleListUsersPagination(update: ProcessedUpdate, bot: TelegramBot, chat: Chat) {
    ...
    // Get message ID from the callback query
    ...
    // Update session with new page and message ID
